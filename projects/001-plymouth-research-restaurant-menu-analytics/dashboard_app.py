...
st.set_page_config(
    ...
    page_icon="🍽️",
    layout="wide",
    initial_sidebar_state="expanded"  # Expanded by default for Analytics Dashboard
)
...
def main():
    ...
    view_mode = st.sidebar.radio(
        ...
        index=0,  # Default to Analytics Dashboard
    ...
    if view_mode == "🗺️ Full-Screen Map":
        st.sidebar.divider()
        st.sidebar.subheader("🗺️ Map Controls")

        show_hygiene_filter_map = st.sidebar.checkbox("Color by Hygiene Rating", value=True)

        if show_hygiene_filter_map:
            min_hygiene_map = st.sidebar.select_slider(
    ...
    # ========================================================================
    # View Selector - Analytics Dashboard or Full-Screen Map
    # ========================================================================
    ...
    if view_mode == "🗺️ Full-Screen Map":
        ...
        if restaurants_with_gps.empty:
            ...
            st.markdown(f"### 📍 Showing {len(restaurants_with_gps)} restaurants with GPS coordinates")

            # Apply map-specific hygiene filter
            if show_hygiene_filter_map:
                restaurants_with_gps = restaurants_with_gps[restaurants_with_gps['hygiene_rating'] >= min_hygiene_map]

            # Prepare map data (include all relevant columns)
            ...
            # Apply circular spread to near-duplicate coordinates
            ...
            # Create tooltip with comprehensive info
            ...
            # Display map with maximum height to fill browser
            st.pydeck_chart(deck, use_container_width=True, height=700)

            # Compact legend
            st.markdown("#### 🎨 Legend")
            legend_cols = st.columns(5)
            legend_cols[0].markdown("🟢 **5★** Very Good")
            legend_cols[1].markdown("🟢 **4★** Good")
            legend_cols[2].markdown("🟡 **3★** Satisfactory")
            legend_cols[3].markdown("🟠 **2★** Needs Improvement")
            legend_cols[4].markdown("🔴 **1★** Major Improvement")
        ...
        with tab1:
            ...
            st.markdown("Interactive map showing restaurant locations with hygiene ratings, licensing, and business rates information.")
            ...
            if restaurants_with_gps.empty:
                ...
                # Map controls in columns
                col_filter1, col_filter2 = st.columns(2)

                with col_filter1:
                    show_hygiene_filter = st.checkbox("Color by Hygiene Rating", value=True, key="tab_hygiene_filter")

                with col_filter2:
                    map_style_tab = st.selectbox(
                        "Map Style",
                        options=["Open Street Map", "Satellite"],
                        index=0,
                        key="tab_map_style"
                ...
                # Apply hygiene filter if enabled
                if show_hygiene_filter:
                    min_hygiene = st.slider(
                    ...
                    restaurants_with_gps = restaurants_with_gps[
                        (restaurants_with_gps['hygiene_rating'] >= min_hygiene) |
                        (restaurants_with_gps['hygiene_rating'].isna())
                    ]
