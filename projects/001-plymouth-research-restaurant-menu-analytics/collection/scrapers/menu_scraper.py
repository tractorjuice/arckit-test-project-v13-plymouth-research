...
class MenuScraper:
    ...
    def __init__(self, db_connection=None, min_delay_seconds: float = 5.0):
        """
        ...
        self.rate_limiter = DomainRateLimiter(min_delay_seconds=min_delay_seconds)
    ...
    def _parse_menu_html(self, html: str, source_url: str) -> List[Dict]:
        ...
        Restaurant-specific parsing logic should be added.
        ...
        # PLACEHOLDER: Basic extraction (will be enhanced with restaurant-specific parsers)
    ...
    def _log_to_database(self, log_data: Dict):
        ...
        try:
            self.db_connection.log_scraping_attempt(log_data)
            logger.debug(f"📝 Logged scraping attempt to database: {log_data['url']}")
        except Exception as e:
